...
public class VehicleDbHelper {
	...
	private static String query_search_by_make_model = "select * from tbl_vehicle where make=? and model=?";
	...
	protected void create(Connection connection, Vehicle car)
			throws MetaException {
		PreparedStatement preparedStatement = null;
		try {
	...
	protected void edit(Connection connection, Vehicle car) throws MetaException {
		PreparedStatement preparedStatement = null;
		try {
			preparedStatement = connection.prepareStatement(edit_car_query);
			preparedStatement.setString(1, car.getMake());
			preparedStatement.setString(2, car.getModel());
			preparedStatement.setInt(3, car.getEnginInCC());
			preparedStatement.setInt(4, car.getFuelCapacity());
	...
	public static List<String> getDistinctMake(Connection connection)
			throws MetaException {
		String make = "";
	...
	public static List<Vehicle> getVehicleMake(Connection connection,
			Vehicle make) throws MetaException {
		Vehicle vehicle;
	...
	public static Car getVehicleId(Connection connection,Car car) throws MetaException {
		Car newCar = new Car();
		ResultSet resultSet = null;
		if (connection != null) {
			PreparedStatement preparedStatement = null;
			try {
				...
				newCar.setPowerSteering(Boolean.parseBoolean(resultSet
						.getString("power_steering")));
				newCar.setAccessoryKit(Boolean.parseBoolean(resultSet
						.getString("accessory_kit")));
	...
	public Car getCarByMakeModel(Connection connection, Car car)
			throws MetaException {

		Car returnValue = null;

		if (connection != null && car != null && car.getMake().equals(null)
				&& car.getModel().equals(null)) {
			PreparedStatement preparedStatement = null;
			try {
				preparedStatement = connection
						.prepareStatement(query_search_by_make_model);
				preparedStatement.setString(1, car.getMake());
				preparedStatement.setString(2, car.getModel());
				ResultSet resultSet = preparedStatement.executeQuery();
				while (resultSet.next()) {
					returnValue = new Car();
					returnValue.setVehicle_id(resultSet.getInt("vehicle_id"));
					returnValue.setMake(resultSet.getString("make"));
					returnValue.setModel(resultSet.getString("model"));
					break;
				}
			} catch (SQLException e) {
				System.out.println("Error while getting car by make,model "
						+ car.getMake());
				throw new MetaException(
						"Error while getting car by make,model "
								+ car.getMake() + ", [" + e.getMessage() + "]");
			}
